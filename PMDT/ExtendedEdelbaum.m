...
[semiMajor,inclination, ~, tSeg, deltaV_Edel,~,~,beta,f] =...
    Edelbaum(a0, af, inc0,incf,param);
...
for i = 1:length(tSeg)-1
    ...
    if param.eclipses == true
        weight = shadowArc(semiMajor(i), inclination(i), RAAN(i), tSeg(i)+ param.TOFbefore,param);
        weight = min([param.dutyRatio,weight]);
    else
    ...
    if param.drag == true
        ...
        a_d_kplus1  = param.mu./(V0^2 + f_d_kplus1^2*tSeg(i+1).^2 -2*V0*f_d_k*tSeg(i+1)); %m
       ...
       if a_drag_pchange > param.a_pchangeLimit && abs(a_d_k - a_d_kplus1)<semiMajor(i)
            ...
            DV_tospend= 0.9766*sqrt(Vstart^2 + Vf^2 - 2*Vstart*Vf*cos(pi/2*Deltai));
