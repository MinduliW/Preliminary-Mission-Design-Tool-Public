function [result,RAANs,Total_TOF,Total_dV,fuelconsump] = PMDT(x, debrisID, param)
...
mf(1) = param.m0  - mass_after;
 ...
 if param.plots == true
     ...
     plot(tSeg/param.TU, a0/1e3*ones(size(tSeg)), 'r--');
     plot(tSeg/param.TU, af/1e3*ones(size(tSeg)),'g--');
     p = plot(tSeg/param.TU,semiMajor/1e3,'c');
   
     
     xlim([min(param.TOFbefore+tSeg/param.TU),max(param.TOFbefore+tSeg/param.TU)]);
     
     plot_latex(p, 'time(days)', 'a (km)','', 'Leg 1' ,...
         {'Initial','Target','Edelbaum', 'Ruggeiro', 'Lyupunov'});
     
     subplot(3,1,2); hold on;
     plot(tSeg/param.TU, rad2deg(inc0*ones(size(tSeg))), 'r--');
     plot(tSeg/param.TU, rad2deg(incf*ones(size(tSeg))),'g--');
     p = plot(tSeg/param.TU,inclination*180/pi, 'c');
     ylim([inc0*180/pi-1, inc0*180/pi+1]);
     xlim([min(param.TOFbefore+tSeg/param.TU),max(param.TOFbefore+tSeg/param.TU)]);
     ...
     p = plot(tSeg/param.TU,RAAN*180/pi, 'c');
     xlim([min(param.TOFbefore+tSeg/param.TU),max(param.TOFbefore+tSeg/param.TU)]);
...
%param.guidance = false;
    ...
    if param.plots == true
        ...
        figure;
        subplot(3,1,1);hold on;
        plot(param.TOFbefore+tSeg/param.TU, a0/1e3*ones(size(tSeg)), 'r--');
        plot(param.TOFbefore+tSeg/param.TU, af/1e3*ones(size(tSeg)),'g--');
        p = plot(param.TOFbefore+tSeg/param.TU,semiMajor/1e3, 'c');
        ...
        subplot(3,1,2); hold on;
        plot(param.TOFbefore+tSeg/param.TU, rad2deg(inc0*ones(size(tSeg))), 'r--');
        plot(param.TOFbefore+tSeg/param.TU, rad2deg(incf*ones(size(tSeg))),'g--');
        p = plot(param.TOFbefore+tSeg/param.TU,inclination*180/pi, 'c');
