function [Total_dv1,TOF1,targetRAAN,waittime,rrs,fg, tofg] = processUpLegApp...
    (app,x , a0,inc0,RAAN0 , af, incf, RAANf,param)
rrs = 0;
...
if param.plots == true
    ...
    ax1 = nexttile(t);
    hold(ax1,'on');
    plot(ax1,tT, a0/1e3*ones(size(tSeg)), 'r--');
    plot(ax1,tT, af/1e3*ones(size(tSeg)),'g--');
    plot(ax1,tL1,leg1.a/1e3, 'c');
    plot(ax1,tw,wait.a/1e3, 'c' )
    p = plot(ax1,tL2,leg2.a/1e3, 'c');
    ...
    ax2 = nexttile(t);
    hold(ax2,'on');
    plot(ax2,tT, rad2deg(param.x0(2)*ones(size(tSeg))), 'r--');
    plot(ax2,tT, rad2deg(param.xf(2)*ones(size(tSeg))),'g--');
    plot(ax2,tL1,leg1.inc*180/pi ,'c');
    plot(ax2,tw,leg1.inc(end)*180/pi*ones(size(tw)) ,'c')
    p = plot(ax2,tL2,leg2.inc*180/pi, 'c');
    
    ax2.XLim = ([min(tT),max(tT)]);
    ...
    ax3 = nexttile(t);
    hold(ax3,'on');
    plot(ax3,tT, rad2deg(Omega_x0), 'r--');
    plot(ax3,tT, rad2deg(Omega_xf),'g--')
    plot(ax3,tL1,wrapTo2Pi(leg1.RAAN)*180/pi,'c')
    plot(ax3,tw,Omega_tw*180/pi, 'c');
    p = plot(ax3,tL2,wrapTo2Pi(leg2.RAAN)*180/pi, 'c');
